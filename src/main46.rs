...
fn rob_houses_memoized(nums: &Vec<i32>, memo: &mut HashMap<String, i32>, curr_house: usize,
                       prev_selected: bool) -> i32 {
    ...
    let mut max_sum = 0;
    if prev_selected {
        max_sum = rob_houses_memoized(nums, memo, curr_house + 1, false);
    } else {
        max_sum = max(
