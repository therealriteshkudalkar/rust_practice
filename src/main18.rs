...
fn count_vowel_strings_for_character<'a>(map: &mut HashMap<&'a str, i32>, vow_ar: &Vec<char>, index: usize, n: i32) -> i32 {
    if n == 1 {
        return 1;
    }
    let va = format!("{}, {n}", vow_ar[index]);
    let v: &'a str = va.clone().as_str();
